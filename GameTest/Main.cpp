...
bool isDirectionLeft() {
    return ballDir == 5 || ballDir == 4 || ballDir == 3;
}
    ...
    */
    ...
    do {
        ...
        if (ballBuffer == 40) {
            ...
            if (ball[0][0] == 2) {
                for (int i = 0; i < 4; i++) {
                    if (players[0][i] == ball[0][1]) {
                        topCollision = players[0][i];
                    }
                    if (players[0][i] == ball[3][1]) {
                        bottomCollision = players[0][i];
                    }
                }
            } else if (ball[1][0] == 13) {
                for (int i = 0; i < 4; i++) {
                    if (players[1][i] == ball[0][1]) {
                        topCollision = players[1][i];
                    }
                    if (players[1][i] == ball[3][1]) {
                        bottomCollision = players[1][i];
                    }
                }
            }
            //player 0:
            if (((topCollision == -1 && bottomCollision == players[0][3])
                || (topCollision == players[0][3] && bottomCollision == players[0][2])) && isDirectionLeft())
                ballDir = 0;
            else if (((topCollision == players[0][0] && bottomCollision == -1)
                || (topCollision == players[0][1] && bottomCollision == players[0][0])) && isDirectionLeft())
                ballDir = 2;
            else if (topCollision == players[0][2] && bottomCollision == players[0][1] && isDirectionLeft())
                ballDir = 1;
            //player 1:
            else if (((topCollision == -1 && bottomCollision == players[1][3])
                || (topCollision == players[1][3] && bottomCollision == players[1][2])) && !isDirectionLeft())
                ballDir = 5;
            else if (((topCollision == players[1][0] && bottomCollision == -1)
                || (topCollision == players[1][1] && bottomCollision == players[1][0])) && !isDirectionLeft())
                ballDir = 3;
            else if (topCollision == players[1][2] && bottomCollision == players[1][1] && !isDirectionLeft())
                ballDir = 4;
            // BALL MOVEMENT
            switch (ballDir) {
                case 0:
                    if (ball[1][0] == 15 && ball[1][1] == 15) score(false);
                    else if (ball[1][0] == 15) score(false);
